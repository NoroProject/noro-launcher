...
pub fn cape_panel(ui: &LauncherUI, cx: &mut Cx) -> AnyElement {
    ...
    panel().p(px(14.)).flex_1().min_h_0().h_full().overflow_hidden().flex().flex_col().gap(px(12.))
        .child(header_row())
        .child(div().id("capes-scroll").flex_1().min_h_0().overflow_y_scroll().pb(px(12.))
...
fn header_row() -> AnyElement {
    div().flex().items_center().justify_between()
        .child(div().font_family(FONT_PIXEL_ALT).text_size(px(13.)).font_weight(FontWeight::BOLD).text_color(rgb(CTA)).child("Выбор плаща"))
        .into_any_element()
}
...
fn no_cape_card(selected: bool, cx: &mut Cx) -> AnyElement {
    ...
    div().id("no-cape-tile").w(gpui::relative(0.315)).p(px(4.))
        .bg(rgb(BG_CARD)).rounded(px(R_SM)).border_1().border_color(rgb(border_clr))
        ...
        .child(
            div().w(px(72.)).h(px(96.)).flex().flex_col().items_center().justify_center().gap(px(6.))
                ...
                .child(div().font_family(FONT_PIXEL_ALT).text_size(px(9.)).text_color(rgb(TEXT_MUTED)).child("Снять плащ"))
        )
        .child(
            div().w_full().px(px(4.)).truncate().text_center()
                .font_family(FONT_PIXEL_ALT).text_size(px(10.)).font_weight(FontWeight::BOLD)
        ...
        .child(
            if selected {
                div().w_full().py(px(3.)).rounded(px(R_SM)).bg(rgb(CTA))
                    ...
                    .font_family(FONT_PIXEL_ALT).text_size(px(9.)).font_weight(FontWeight::BOLD).text_color(rgb(ON_CTA))
                ...
                div().w_full().py(px(3.)).rounded(px(R_SM)).bg(rgb(BG_INPUT)).border_1().border_color(rgb(BORDER))
                    ...
                    .flex().items_center().justify_center()
                    .font_family(FONT_PIXEL_ALT).text_size(px(9.)).font_weight(FontWeight::BOLD).text_color(rgb(TEXT_PRIMARY))
                    .child("Снять").into_any_element()
            }
        )
        .into_any_element()
}

fn cape_tile_card(ui: &LauncherUI, cape: &schema::CapeRow, selected: bool, cx: &mut Cx) -> AnyElement {
    let id = cape.id;
    ...
    let img_el = if let Some(loaded_img) = ui.cape_images.get(&id) {
        ...
        div().w(px(72.)).h(px(96.)).flex().items_center().justify_center().child(ic("layers", 24., if selected { CTA } else { TEXT_MUTED })).into_any_element()
    };

    div().id(card_id).w(gpui::relative(0.315)).p(px(4.))
        .bg(rgb(BG_CARD)).rounded(px(R_SM)).border_1().border_color(rgb(border_clr))
        .hover(|s| s.bg(rgb(BG_INPUT))).cursor_pointer()
        .flex().flex_col().items_center().gap(px(4.))
