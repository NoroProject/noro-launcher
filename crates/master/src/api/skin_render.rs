...
pub fn render_bust(skin: &DynamicImage, scale: u32, overlay: bool) -> Vec<u8> {
    ...
    if overlay {
        copy_region_alpha(skin, &mut canvas, 40, 8, 8, 8, 4, 0);
    }

    copy_region(skin, &mut canvas, 20, 20, 8, 12, 4, 8);
    if overlay {
        copy_region_alpha(skin, &mut canvas, 20, 36, 8, 12, 4, 8);
    }

    copy_region(skin, &mut canvas, 44, 20, 4, 12, 0, 8);
    if overlay {
        copy_region_alpha(skin, &mut canvas, 44, 36, 4, 12, 0, 8);
    }

    if !is_legacy {
        copy_region(skin, &mut canvas, 36, 52, 4, 12, 12, 8);
        if overlay {
            copy_region_alpha(skin, &mut canvas, 52, 52, 4, 12, 12, 8);
        }
    } else {
...
pub fn render_body(skin: &DynamicImage, scale: u32, overlay: bool) -> Vec<u8> {
    ...
    if overlay {
        copy_region_alpha(skin, &mut canvas, 4, 36, 4, 12, 4, 20);
    }

    if !is_legacy {
        copy_region(skin, &mut canvas, 20, 52, 4, 12, 8, 20);
        if overlay {
            copy_region_alpha(skin, &mut canvas, 4, 52, 4, 12, 8, 20);
        }
    } else {
