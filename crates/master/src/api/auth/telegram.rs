...
pub async fn telegram_callback(
    ...
    let id = params.get("id").cloned().ok_or_else(|| AppError::BadRequest("Missing id".into()))?;
    ...
    let identity = RemoteIdentity { id, username, avatar };
...
fn process_telegram_auth(req: &TelegramAuthReq, bot_token: &str) -> AppResult<(RemoteIdentity, String)> {
        ...
        if let Some(map) = verify_init_data(init_data, bot_token) {
            if let Some(user_json) = map.get("user") {
                if let Ok(val) = serde_json::from_str::<serde_json::Value>(user_json) {
                    ...
                    let username = val.get("username").and_then(|s| s.as_str())
                        .or_else(|| val.get("first_name").and_then(|s| s.as_str()))
                        .unwrap_or("TelegramUser").to_string();
                    let avatar = val.get("photo_url").and_then(|s| s.as_str()).map(String::from);
                    return Ok((RemoteIdentity { id: id.clone(), username, avatar }, id));
                }
            }
        }
    }
    ...
    if let Some(ref v) = req.id { map.insert("id".into(), v.clone()); }
    if let Some(ref v) = req.first_name { map.insert("first_name".into(), v.clone()); }
    if let Some(ref v) = req.last_name { map.insert("last_name".into(), v.clone()); }
    if let Some(ref v) = req.username { map.insert("username".into(), v.clone()); }
    if let Some(ref v) = req.photo_url { map.insert("photo_url".into(), v.clone()); }
    if let Some(ref v) = req.auth_date { map.insert("auth_date".into(), v.clone()); }
    if let Some(ref v) = req.hash { map.insert("hash".into(), v.clone()); }

    if verify_widget_hash(&map, bot_token) {
        let id = req.id.clone().ok_or_else(|| AppError::BadRequest("Missing id".into()))?;
        let username = req.username.clone()
            .or_else(|| req.first_name.clone())
            .unwrap_or_else(|| format!("tg_{id}"));
        let avatar = req.photo_url.clone();
        Ok((RemoteIdentity { id: id.clone(), username, avatar }, id))
    } else {
        Err(AppError::Unauthorized("Invalid Telegram auth hash signature".into()))
    }
}

pub fn verify_widget_hash(params: &BTreeMap<String, String>, bot_token: &str) -> bool {
    let Some(hash) = params.get("hash") else { return false; };
    if let Some(auth_date_str) = params.get("auth_date") {
        if let Ok(auth_date) = auth_date_str.parse::<i64>() {
            let now = chrono::Utc::now().timestamp();
            if (now - auth_date).abs() > 86400 { return false; }
        }
    }
    ...
    let Ok(mut mac) = HmacSha256::new_from_slice(&secret_key) else { return false; };
...
pub fn verify_init_data(init_data: &str, bot_token: &str) -> Option<BTreeMap<String, String>> {
    ...
    for pair in init_data.split('&') {
        if let Some((k, v)) = pair.split_once('=') {
            let decoded_v = urlencoding::decode(v).unwrap_or(std::borrow::Cow::Borrowed(v)).to_string();
    ...
    if let Some(auth_date_str) = params.get("auth_date") {
        if let Ok(auth_date) = auth_date_str.parse::<i64>() {
            let now = chrono::Utc::now().timestamp();
            if (now - auth_date).abs() > 86400 { return None; }
        }
    }
    ...
    let Ok(mut secret_mac) = HmacSha256::new_from_slice(b"WebAppData") else { return None; };
    secret_mac.update(bot_token.as_bytes());
    let secret_key = secret_mac.finalize().into_bytes();

    let Ok(mut mac) = HmacSha256::new_from_slice(&secret_key) else { return None; };
    mac.update(check_string.as_bytes());
    let calculated_hash = hex::encode(mac.finalize().into_bytes());
    if calculated_hash.eq_ignore_ascii_case(&hash) { Some(params) } else { None }
}
...
mod tests {
    ...
    fn test_telegram_widget_hash_verification() {
        ...
        map.insert("auth_date".into(), chrono::Utc::now().timestamp().to_string());
