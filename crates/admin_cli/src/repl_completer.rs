...
impl ReplHelper {
    pub fn new(cache: Arc<RwLock<DynamicCache>>) -> Self {
        ...
        subcommands.insert(
            "file",
            vec!["list", "upload", "download", "delete", "move", "edit"],
        );
        subcommands.insert(
            "mod",
            vec![
        ...
        subcommands.insert(
            "game",
            vec!["exec", "console", "power", "kick", "tell", "announce"],
        );
...
impl Completer for ReplHelper {
    ...
    fn complete(
        ...
        let start = if ends_with_space {
            ...
            pos.saturating_sub(filter.len())
        };

        if let Ok(c) = self.cache.read() {
            let target_items = if first == "user"
                && (sub == "add-role" || sub == "set-role" || sub == "remove-role")
                && (words.len() == 3 || (words.len() == 2 && ends_with_space))
            {
                if words.len() == 3 || (words.len() == 2 && !ends_with_space) {
                    &c.role_items
                } else {
                    &c.user_items
                }
