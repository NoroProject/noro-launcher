...
pub async fn run(c: &Client, cmd: UserCmd) -> Result<()> {
    let v = match cmd {
        ...
        UserCmd::AddRole { id, role_id } => {
            ...
            c.post(&format!("/api/admin/users/{uid}/roles/{rid}"), json!({}))
                .await?
        }
        UserCmd::RemoveRole { id, role_id } => {
            let uid = resolve_user_id(c, &id).await?;
            let rid = resolve_role_id(c, &role_id).await?;
            c.delete(&format!("/api/admin/users/{uid}/roles/{rid}"))
                .await?
        }
        UserCmd::AddPerm { id, permission } => {
            let uid = resolve_user_id(c, &id).await?;
            c.post(
                &format!("/api/admin/users/{uid}/permissions"),
        ...
        UserCmd::RemovePerm { id, permission } => {
            let uid = resolve_user_id(c, &id).await?;
            c.delete(&format!(
                "/api/admin/users/{uid}/permissions/{}",
        ...
        UserCmd::SetCape { id, cape_id } => {
            let uid = resolve_user_id(c, &id).await?;
            c.put(
                &format!("/api/admin/users/{uid}/cape"),
        ...
        UserCmd::SetSkin { id, file } => {
            let uid = resolve_user_id(c, &id).await?;
            c.upload_image(&format!("/api/admin/users/{uid}/skin"), &file, "skin")
                .await?
        }
        UserCmd::FetchSkin { id, mc_username } => {
            ...
            let profile_url =
                format!("https://api.mojang.com/users/profiles/minecraft/{target_name}");
            let resp: serde_json::Value = reqwest::get(&profile_url).await?.json().await?;
            let uuid_hex = resp
                .get("id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| anyhow::anyhow!("Player not found on Mojang API"))?;
            let sess_url =
                format!("https://sessionserver.mojang.com/session/minecraft/profile/{uuid_hex}");
                ...
                for p in props {
                    if p.get("name").and_then(|s| s.as_str()) == Some("textures") {
                        if let Some(val_b64) = p.get("value").and_then(|s| s.as_str()) {
                            use base64::Engine;
                            let decoded =
                                base64::engine::general_purpose::STANDARD.decode(val_b64)?;
            ...
            let skin_url =
                skin_url.ok_or_else(|| anyhow::anyhow!("No skin texture found for player"))?;
            ...
            let res = c
                .upload_image(&format!("/api/admin/users/{uid}/skin"), &temp_path, "skin")
                .await?;
        ...
        UserCmd::SkinPresets { id } => {
            let uid = resolve_user_id(c, &id).await?;
            c.get(&format!("/api/admin/users/{uid}/skin-presets"))
                .await?
        }
        UserCmd::SavePreset {
            id,
            ...
            let uid = resolve_user_id(c, &id).await?;
            c.post(
                &format!("/api/admin/users/{uid}/skin-presets"),
                json!({ "name": name, "skin_url": skin_url, "slim": slim }),
            )
            .await?
        }
        UserCmd::SelectPreset { id, skin_url, slim } => {
            let uid = resolve_user_id(c, &id).await?;
            c.post(
                &format!("/api/admin/users/{uid}/skin-presets/select"),
                json!({ "skin_url": skin_url, "slim": slim }),
            )
            .await?
        }
        UserCmd::DeletePreset { id, preset_id } => {
            let uid = resolve_user_id(c, &id).await?;
            c.delete(&format!("/api/admin/users/{uid}/skin-presets/{preset_id}"))
                .await?
        }
    };
    print_json(&v);
        ...
        .or_else(|| v.get("items").and_then(|i| i.as_array()))
    {
        for u in arr {
            ...
            if username.eq_ignore_ascii_case(input) {
                if let Some(id) = u.get("id").and_then(|s| s.as_str()) {
        ...
        .or_else(|| v.get("items").and_then(|i| i.as_array()))
    {
        for r in arr {
            ...
            if name.eq_ignore_ascii_case(input) {
                if let Some(id) = r.get("id").and_then(|s| s.as_str()) {
                    return Ok(id.to_string());
                }
            }
        }
