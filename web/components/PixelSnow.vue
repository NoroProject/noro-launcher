...
/**
 * Пиксельный снег на фоне — компонент Vue Bits (vue-bits.dev), MIT.
 *
 * Шейдер и параметры авторские, изменены три вещи, без которых полноэкранный
 * рейммарчинг на каждой странице обходится дорого:
 *  - при системном «уменьшить движение» кадр рисуется один раз и замирает;
 *  - в скрытой вкладке цикл останавливается, а не крутится в фоне;
 *  - слой помечен `aria-hidden`: для читалки это украшение, а не содержимое.
 */
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          // Прозрачность равна яркости, а не 1.0 как в исходнике. Иначе
          // дальняя тусклая снежинка оставалась непрозрачной и рисовала
          // почти чёрный квадрат поверх фона — на тёмной теме это читалось
          // как грязь и битые пиксели, а не как снег вдали.
          float v = pow(intensity, uGamma);
...
const setupFn = () => {
  ...
  // Системная настройка «уменьшить движение» — не про вкус, а про людей,
  // которым от анимации физически плохо. Им рисуем один статичный кадр.
  ...
  // Вкладка в фоне — кадры считать незачем: браузер их не показывает, а
  // видеокарта всё равно занята.
  const handleVisibility = () => {
    cancelAnimationFrame(animationRef.value)
    if (!document.hidden && !stillOnly) animate()
  }
