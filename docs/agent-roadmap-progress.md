...
### Полная синхронизация веб-панели и бэкенда, 2026-08-19

Проведён аудит всех эндпоинтов бэкенда и связанных компонентов UI:

1. **Интерфейс внутриигровых действий (`/admin/clients/[id]/game/[gsid]`):**
   - Создан компонент `GameserverActions.vue` и добавлена вкладка **In-Game Actions** для кика игроков (`/api/admin/game/kick`), отправки личных сообщений (`/api/admin/game/tell`) и трансляции объявлений (`/api/admin/game/announce`).
2. **Панель автомодерации чата (`/admin/automod`):**
   - Создана новая страница управления фильтрами чата `automod.vue` для настройки правил (`ad`, `word`, `caps`, `flood`), выбора режима (`deny`, `escalate`, `punish`, `shadow`), привязки кода из `RuleCatalog`, редактирования белых списков, порогов и временных окон.
   - Ссылка на панель добавлена в боковое меню `NoroSidebar.vue`.
3. **Управление репортами (`/admin/reports`):**
   - В UI репортов добавлено принятие жалобы в работу (`POST /api/admin/reports/{id}/claim`) и отображение назначенного модератора (`claimed_by_username`).
4. **График активности (Heatmap):**
   - Зарегистрирован эндпоинт `GET /api/me/activity-heatmap` с объединением прошедших дней и расчётом минут текущей открытой сессии.
